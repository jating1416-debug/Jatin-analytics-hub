...
export default function PostProcessor({ html }: { html: string }) {
  useEffect(() => {
    ...
    // Ye har table ko clean karta hai: attributes hatao, first row -> th,
    // alignment fix. CSS (globals) design deta hai - ye structure fix karta hai.
    try {
      body.querySelectorAll('table').forEach((tbl) => {
        // 1. purane inline attributes hatao (CSS !important design lega)
        ['border', 'cellpadding', 'cellspacing', 'width', 'style'].forEach((attr) => tbl.removeAttribute(attr));
        // 2. pehli row ko <th> banao (agar pure td hai)
        ...
        // 3. agar thead nahi hai to banao
    ...
    quizBlocks.forEach((block, bi) => {
      ...
      questions.forEach((q, qi) => {
        ...
        q.opts.forEach((opt, oi) => {
          ...
          html += `<div class="quiz-opt ${cls}" data-qi="${qi}" data-oi="${oi + 1}" data-quiz="${bi}" style="display:flex;align-items:center;gap:10px;padding:8px 12px;margin:5px 0;border:1px solid var(--border);border-radius:10px;cursor:${saved ? 'default' : 'pointer'};font-size:0.88rem;${
      ...
      // replace quiz block (last occurrence processing)
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (submitBtn) {
        ...
        const line = (block.querySelector(`.quiz-opt[data-qi="${qi}"]`)?.parentElement?.innerHTML || '');
        // find answer: stored in data-ans on block
        const ansEl = block.querySelector(`.quiz-opt[data-qi="${qi}"][data-oi]`);
        const allOpts = block.querySelectorAll(`.quiz-opt[data-qi="${qi}"]`);
        let correct = -1;
        // answer decode: options marked with data-ans attr
        allOpts.forEach((o) => {
